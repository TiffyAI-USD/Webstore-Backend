...
app.get('/view/:handle', async (req, res) => {
  try {
    ...
    <head>
        ...
        <style>
            :root { --accent: #00d4ff; --bg: #0f0f0f; --card: #1a1a1a; --text: #ffffff; --gold: #ffcc00; }
            ...
            .qty-val { font-weight: 900; font-size: 1.2rem; min-width: 35px; text-align: center; color: #fff; }

            /* WHATSAPP SERVICE BUTTON */
            .wa-service-btn { width: 100%; margin-top: 15px; cursor: pointer; transition: 0.2s; display: block; }
            .wa-service-btn:hover { transform: scale(1.03); }
            ...
            function renderMenu() {
                ...
                storeData.menu.forEach((section, sIdx) => {
                    ...
                    section.items.forEach((item, iIdx) => {
                        ...
                        let controls = '';
                        if(item.price == 0) {
                            const waMsg = encodeURIComponent("Hi, I am interested in " + item.name);
                            controls = '<img src="../../WhatsApp.png" class="wa-service-btn" onclick="window.open(\\'https://wa.me/'+storeData.wa+'?text='+waMsg+'\\', \\'_blank\\')">';
                        } else {
                            hasCartItems = true;
                            controls = \`
                                <div class="cart-controls">
                                    <button class="btn-cart btn-empty \${qty > 0 ? '' : 'hidden-btn'}" onclick="updateCart('\${key}', \${item.price}, -1, '\${item.name}')">EMPTY CART</button>
                                    <span class="qty-val" id="qty-\${key}">\${qty}</span>
                        ...
                        card.innerHTML = \`
                            ...
                            <div class="item-body">
                                <div class="item-header">
                                    ...
                                    <span class="item-price">\${item.price == 0 ? 'NEGOTIABLE' : storeData.curr + ' ' + item.price}</span>
                                </div>
                ...
                // Hide bottom order bar if there are no items with prices
                document.getElementById('bottom-order-bar').style.display = hasCartItems ? 'flex' : 'none';
            }
