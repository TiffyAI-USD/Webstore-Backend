...
/* ================== THE VIBE SHORTENER (INTERNAL REDIRECTOR) ================== */
// This serves the "Stamp" to WhatsApp/Telegram instantly with ZERO 3rd parties.
app.get('/s/:handle', async (req, res) => {
  try {
    const check = await pool.query('SELECT config_data FROM stores WHERE handle = $1', [req.params.handle]);
    if (check.rows.length === 0) return res.status(404).send('Store Not Found');

    const storeData = check.rows[0].config_data;
    const businessName = storeData.businessName || 'Retail OS';
    const tagline = storeData.tagline || 'Professional Web Store';
    let stampImage = storeData.banner || '';
    if (stampImage && !stampImage.startsWith('http')) stampImage = 'https://' + stampImage;
    ...
    <head>
        ...
        <meta property="og:url" content="https://webstore-api-m8sr.onrender.com/s/${req.params.handle}">
        <meta property="og:image" content="${stampImage}">
        <meta name="twitter:card" content="summary_large_image">
        <meta name="twitter:image" content="${stampImage}">
        <script>window.location.href = '/view/${req.params.handle}';</script>
        <style>body{background:#000;display:flex;justify-content:center;align-items:center;height:100vh;color:#444;font-family:sans-serif;} .loader{border:3px solid #333;border-top:3px solid #00d4ff;border-radius:50%;width:30px;height:30px;animation:spin 1s linear infinite;} @keyframes spin{0%{transform:rotate(0deg);}100%{transform:rotate(360deg);}}</style>
    </head>
    <body><div class="loader"></div></body>
    </html>`);
  } catch (err) { res.redirect('/view/' + req.params.handle); }
});
...
app.get('/view/:handle', async (req, res) => {
  try {
    ...
    let stampImage = storeData.banner || '';
    if (stampImage && !stampImage.startsWith('http')) {
        stampImage = 'https://' + stampImage;
    }
    ...
    <body>
        ...
        <script>
            ...
            function shareProduct(n, p) {
                const storeTitle = storeData.businessName;
                // VIBE UPGRADE: Shares your own short link instead of the long view link
                const currentUrl = window.location.origin + '/s/' + handle;
                const msg = 'Check out this ' + n + (p > 0 ? ' for ' + storeData.curr + ' ' + p : '') + ' on ' + storeTitle;
