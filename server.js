...
app.get('/view/:handle', async (req, res) => {
  try {
    ...
    if (!store.is_active || new Date(store.trial_expires) < now) {
        return res.send('<div style="text-align:center;padding:100px;background:#000;color:red;height:100vh;font-family:sans-serif;"><h1>STORE SUSPENDED</h1><p>The 7-day trial has ended or store is inactive.</p></div>');
    }

    // Embedding the storefront with the "Millimeter Wider" Banner
    res.send(`
    <!DOCTYPE html>
    <html lang="en">
    <head>
        ...
        <style>
            ...
            .order-bar { position: fixed; bottom: 0; left: 0; width: 100%; background: #111; border-top: 1px solid #333; padding: 20px; display: flex; justify-content: space-between; align-items: center; z-index:1000; box-sizing:border-box;}
    ...
    <body>
        ...
        <script>
            ...
            function updateCart(k, p, d, n) {
                ...
                cart[k].qty += d;
                if(cart[k].qty <= 0) { delete cart[k]; document.getElementById('qty-'+k).innerText = "0"; }
                else { document.getElementById('qty-'+k).innerText = cart[k].qty; }
...
app.get('/dashboard/:handle', (req, res) => {
    res.send(\`
        <html>
        <head><meta name="viewport" content="width=device-width, initial-scale=1.0"><style>body{background:#0a0a0a; color:#eee; font-family:sans-serif; padding:20px;} .card{background:#161616; padding:20px; border-radius:15px; margin-bottom:15px; border:1px solid #222;} .accent{color:#00d4ff;} .btn{background:#00d4ff; color:#000; border:none; padding:10px; width:100%; border-radius:8px; font-weight:bold; cursor:pointer;}</style></head>
        <body>
            ...
            <script>
                ...
                async function load() {
                    ...
                    document.getElementById('sales-list').innerHTML = sales.map(s => {
                        revTotal += parseFloat(s.total_amount || 0);
                        return '<div class="card"><b>Total: '+s.total_amount+'</b><br><small>'+new Date(s.created_at).toLocaleString()+'</small><br><button class="btn" style="margin-top:10px;">PRINT RECEIPT</button></div>';
                    }).join('');
                    document.getElementById('count').innerText = sales.length;
                    document.getElementById('rev').innerText = revTotal.toFixed(2);
                }
                load();
            </script>
        </body>
...
/* ================== ADMIN MASTER (For Your Eyes Only) ================== */
app.get('/admin/master', async (req, res) => {
    ...
    const rows = r.rows.map(s => '<tr><td>'+s.handle+'</td><td>'+s.owner_whatsapp+'</td><td>'+s.is_active+'</td><td>'+new Date(s.trial_expires).toLocaleDateString()+'</td></tr>').join('');
    res.send('<html><body style="background:#111;color:#fff;font-family:sans-serif;padding:20px;"><h1>👑 Admin Master</h1><table border="1" style="width:100%;border-collapse:collapse;"><tr><th>Store</th><th>WhatsApp</th><th>Active</th><th>Trial Ends</th></tr>'+rows+'</table></body></html>');
});
...
app.get('/api/store/:handle', async (req, res) => {
    ...
    res.json(r.rows.length ? r.rows[0].config_data : {error: "Store not found"});
});
