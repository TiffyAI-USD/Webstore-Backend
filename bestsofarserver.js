...
// 3. Publishing (Accepts Plan Type)
app.post('/api/publish', async (req, res) => {
  try {
    const { handle, configData, ownerWhatsapp, planType } = req.body;
    await pool.query(`
    ...
    [handle, configData, ownerWhatsapp, planType || 'trial']);
    res.json({ success: true });
